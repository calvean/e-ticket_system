...
def update_user(name, email, password):
    ...
    query = "UPDATE users SET name=%s, password=%s WHERE email=%s"
    values = (name, hashed_password, email)
    cursor.execute(query, values)
    conn.commit()

    # Fetch updated user from database by email
    query = "SELECT id, name, email, password FROM users WHERE email=%s"
    cursor.execute(query, (email,))
    result = cursor.fetchone()
    ...
    # Return updated user data as a dictionary
    if result:
        return {'message': 'User updated successfully!', 'status_code': 200, 'id': result[0], 'name': result[1], 'email': result[2], 'password': result[3]}
    else:
        return {'message': 'Error updating user', 'status_code': 500}
